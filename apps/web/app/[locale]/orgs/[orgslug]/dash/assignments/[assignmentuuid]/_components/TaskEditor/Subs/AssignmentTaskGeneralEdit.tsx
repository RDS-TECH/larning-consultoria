...
export function AssignmentTaskGeneralEdit() {
    const t = useTranslations('assignments.task.general');
    ...
    const validate = (values: any) => {
        ...
        if (values.max_grade_value < 20 || values.max_grade_value > 100) {
            errors.max_grade_value = t('validation.gradeRange');
        }
        return errors;
    };
    ...
    return (
        <FormLayout onSubmit={formik.handleSubmit}>
            <FormField name="title">
                <FormLabelAndMessage label={t('fields.title')} message={formik.errors.title} />
            ...
            <FormField name="description">
                <FormLabelAndMessage label={t('fields.description')} message={formik.errors.description} />
                <Form.Control asChild>
                    <Input
                        onChange={formik.handleChange}
            ...
            <FormField name="hint">
                <FormLabelAndMessage label={t('fields.hint')} message={formik.errors.hint} />
                <Form.Control asChild>
                    <Textarea
                        onChange={formik.handleChange}
            ...
            <FormField name="hint">
                <div className='flex space-x-3 justify-between items-center'>
                    <FormLabelAndMessage label={t('fields.referenceFile')} message={formik.errors.hint} />
                    <div className='flex space-x-1.5 text-xs items-center text-gray-500 '>
                        <Info size={16} />
                        <p>{t('fields.allowedFormats')}</p>
                    </div>
            ...
            <FormField name="max_grade_value">
                <FormLabelAndMessage label={t('fields.maxGradeValue')} message={formik.errors.max_grade_value} />
                <Form.Control asChild>
                    <Input
                        onChange={formik.handleChange}
...
function UpdateTaskRef() {
    ...
    const handleFileChange = async (event: any) => {
        ...
        if (res.success === false) {
            ...
            toast.success(t('toast.referenceFileUpdated'))
            setIsLoading(false)
            setError('')
        }
    }
                        ...
                        <div className='flex flex-col rounded-lg bg-white text-gray-400 shadow-lg nice-shadow px-5 py-3 space-y-1 items-center relative'>
                            ...
                            <div className='flex space-x-2 mt-2'>
                                <Link
                                    ...
                                    download
                                    target='_blank'
                                    className='bg-blue-500 text-white px-3 py-1 rounded-full text-xs font-semibold'>{t('buttons.download')}</Link>
                    ...
                    {isLoading ? (
                        ...
                        <div className="flex justify-center items-center">
                            ...
                            >
                                ...
                                <span>{t('buttons.changeReferenceFile')}</span>
                            </button>
                        </div>
                    )}
