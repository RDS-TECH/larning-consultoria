...
function AssignmentEdit() {
    const t = useTranslations('assignments.editor');
        ...
        <>
            <div className='flex mx-auto mt-5 items-center space-x-4'>
                <div className={`flex text-xs rounded-full px-3.5 py-2 mx-auto font-bold outline outline-1 ${!assignment?.assignment_object?.published ? 'outline-gray-300 bg-gray-200/60' : 'outline-green-300 bg-green-200/60'}`}>
                    {assignment?.assignment_object?.published ? t('status.published') : t('status.unpublished')}
                </div>
                ...
                <ToolTip
                    ...
                    slateBlack
                    sideOffset={10}
                    content={t('tooltips.editDetails')}>
                    <div
                        ...
                        <Pencil size={18} />
                        <p className='text-sm font-bold'>{t('buttons.edit')}</p>
                    </div>
                </ToolTip>
                ...
                <ToolTip
                    ...
                    slateBlack
                    sideOffset={10}
                    content={t('tooltips.previewAsStudent')} >
                    <Link
                        target='_blank'
                        ...
                        <Eye size={18} />
                        <p className=' text-sm font-bold'>{t('buttons.preview')}</p>
                    </Link>
                </ToolTip>
                {assignment?.assignment_object?.published && <ToolTip
                    ...
                    slateBlack
                    sideOffset={10}
                    content={t('tooltips.makeUnavailable')} >
                    <div
                        ...
                        <BookX size={18} />
                        <p className='text-sm font-bold'>{t('buttons.unpublish')}</p>
                    </div>
                </ToolTip>}
                {!assignment?.assignment_object?.published &&
                    <ToolTip
                        ...
                        slateBlack
                        sideOffset={10}
                        content={t('tooltips.makeAvailable')} >
                        <div
                            onClick={() => updateAssignmentPublishState(assignment?.assignment_object?.assignment_uuid)}
                            ...
                            <BookOpen size={18} />
                            <p className=' text-sm font-bold'>{t('buttons.publish')}</p>
                        </div>
                    </ToolTip>}
            </div>
            {isEditModalOpen && (
