...
function AddRole(props: AddRoleProps) {
    const t = useTranslations('users.roles.addRole')
    const tFields = useTranslations('users.roles.addRole.fields')
    const tSections = useTranslations('users.roles.addRole.sections')
    const tPredefined = useTranslations('users.roles.addRole.predefinedRoles')
    const tPermSections = useTranslations('users.roles.addRole.permissionSections')
    const tPermActions = useTranslations('users.roles.addRole.permissionActions')
    const tButtons = useTranslations('users.roles.addRole.buttons')
    const tValidation = useTranslations('users.roles.addRole.validation')
    const tToast = useTranslations('users.roles.addRole.toast')
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            const toastID = toast.loading(tToast('creating'))
            ...
            if (res.status === 200 || res.status === 201) {
                ...
                toast.success(tToast('created'), {id:toastID})
            } else {
                setIsSubmitting(false)
                toast.error(tToast('createError'), {id:toastID})
            }
        },
    })
            ...
            <FormLayout onSubmit={formik.handleSubmit}>
                ...
                <div className="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3 mt-6 pt-6 border-t border-gray-200">
                    ...
                    <Form.Submit asChild>
                        ...
                        >
                            {isSubmitting ? tButtons('creating') : tButtons('createRole')}
                        </button>
                    </Form.Submit>
                </div>
            </FormLayout>
