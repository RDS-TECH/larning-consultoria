...
function OrgUsersAdd() {
    ...
    const t = useTranslations('dashboard.users.add')
    ...
    async function sendInvites() {
        ...
        if (res.status == 200) {
            ...
            setIsLoading(false)
            toast.success(t('toast.sendSuccess'), {id:toastId})
        } else {
            toast.error(t('toast.sendFailed'), {id:toastId})
            setIsLoading(false)
        }
                    ...
                    <div className="ml-10 mr-10 mx-auto bg-white rounded-xl shadow-xs px-4 py-4 anit ">
                        ...
                        <div className="flex space-x-2 mx-auto">
                            <textarea
                                ...
                                className='w-full h-[200px] rounded-md border px-3 py-2 bg-gray-100/40 placeholder:italic placeholder:text-slate-300' placeholder={t('placeholder')} name="" id="" ></textarea>
                        </div>
                        <div className="flex space-x-2 mx-auto my-5 ml-2 items-center space-x-4 justify-between">

                            <div className='flex space-x-2 items-center'>
                                <p className='flex items-center'>{t('inviteCode')} </p>
                                <select
                                ...
                                <ToolTip content={t('inviteCodeTooltip')} sideOffset={8} side="right"><Info className='text-gray-400' size={14} /></ToolTip>
                            </div>
                        ...
                        <div className="flex flex-col bg-gray-50 -space-y-1  px-5 py-3 rounded-md mt-3 mb-3 ">
                            <h1 className="font-bold text-xl text-gray-800">
                                {t('invitedUsers.title')}
                            </h1>
                            <h2 className="text-gray-500  text-md">
                                {' '}
                                {t('invitedUsers.description')}{' '}
                            </h2>
                        </div>
                            ...
                            <table className="table-auto w-full text-left whitespace-nowrap rounded-md overflow-hidden">
                                <thead className="bg-gray-100 text-gray-500 rounded-xl uppercase">
                                    <tr className="font-bolder text-sm">
                                        <th className="py-3 px-4">{t('invitedUsers.table.email')}</th>
                                        <th className="py-3 px-4">{t('invitedUsers.table.signupStatus')}</th>
                                        <th className="py-3 px-4">{t('invitedUsers.table.emailSent')}</th>
                                    </tr>
                                </thead>
                                <>
                                    <tbody className="mt-5 bg-white rounded-md">
                                        {invited_users?.map((invited_user: any) => (
                                            ...
                                            >
                                                ...
                                                <td className="py-3 px-4">{invited_user.pending ? <div className='bg-orange-400 text-orange-100 w-fit px-2 py1 rounded-md'>{t('invitedUsers.status.pending')}</div> : <div className='bg-green-400 text-green-100 w-fit px-2 py1 rounded-md'>{t('invitedUsers.status.signed')}</div>}</td>
                                                <td className="py-3 px-4">{invited_user.email_sent ? <div className='bg-green-600 text-green-100 w-fit px-2 py1 rounded-md'>{t('invitedUsers.status.sent')}</div> : <div className='bg-red-400 text-red-100 w-fit px-2 py1 rounded-md'>{t('invitedUsers.status.no')}</div>}</td>
