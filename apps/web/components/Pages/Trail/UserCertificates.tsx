...
const UserCertificates: React.FC<UserCertificatesProps> = ({ orgslug }) => {
  const t = useTranslations('trail.certificates')
  const session = useLHSession() as any
  const access_token = session?.data?.tokens?.access_token
  const org = useOrg() as any
  const locale = 'pt-BR' // ou use useLocale() do next-intl se quiser suporte multilíngue
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2 text-xs text-gray-600">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span>{t('awarded')} {awardedDate}</span>
                  </div>
