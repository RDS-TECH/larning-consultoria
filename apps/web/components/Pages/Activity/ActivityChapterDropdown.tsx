  ...
  const getActivityTypeLabel = (activityType: string) => {
    switch (activityType) {
      ...
      case 'TYPE_DYNAMIC':
        return t('types.page');
      case 'TYPE_ASSIGNMENT':
        return t('types.assignment');
      default:
        return t('types.learningMaterial');
    }
  };
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        aria-label={t('chapterDropdown.viewAllActivities')}
        title={t('chapterDropdown.viewAllActivities')}
      >
        <ListTree size={17} />
        <span className="text-xs font-bold">{t('chapterDropdown.chapters')}</span>
      </button>
      ...
      {isOpen && (
        <div className={`absolute z-50 mt-2 ${isMobile ? 'right-0 w-[90vw] sm:w-72' : 'right-0 w-72'} max-h-[70vh] cursor-pointer overflow-y-auto bg-white rounded-lg shadow-xl border border-gray-200 py-1 animate-in fade-in duration-200`}>
          <div className="px-3 py-1.5 border-b border-gray-100 flex justify-between items-center">
            <h3 className="text-sm font-semibold text-gray-800">{t('chapterDropdown.courseContent')}</h3>
            <button
                              ...
                              <div className="flex items-center space-x-1.5 w-full">
                                ...
                                {isCurrent && (
                                  <div className="flex items-center space-x-1 text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded-full text-[10px] font-medium animate-pulse">
                                    <span>{t('navigation.current')}</span>
                                  </div>
                                )}
                              </div>
